...
const cycleTrueOrNot = (initial, current) =>
  current.haveCycle ? [...initial, current.name] : [...initial];
...
const sumOfAll = (initial, current) =>
  current > 50 ? current + initial : initial;
...
const productOfEven = (initial, current) =>
  current % 2 === 0 ? current * initial : initial;
...
const hyphenBetweenEven = (num) =>
  [...String(num)]
    .reduce(
      (initial, current) =>
        initial.slice(-1)[0] % 2 === 0 && current % 2 === 0
          ? [...initial, "-", current]
          : [...initial, current],
      []
    )
...
// if vowel toUpperCase and if not vowel toLowerCase
const str = "neOgRaMmeR";
const splitStr = str.split("");
const result = splitStr.reduce((acc, curr) => {
  if (
    curr.toLowerCase().includes("a") ||
    curr.toLowerCase().includes("e") ||
    curr.toLowerCase().includes("i") ||
    curr.toLowerCase().includes("o") ||
    curr.toLowerCase().includes("u")
  ) {
